...
$consulta = "SELECT * FROM  patinetes WHERE id = ${id}";
...
$db = conexionDB();
    ...
    $modelo = $productoId['modelo'];
    $caracteristicas = $productoId['caracteristicas'];
    $precio = $productoId['precio'];
    $potencia = $productoId['potencia'];
    $velocidad = $productoId['velocidad'];
...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $potencia = mysqli_real_escape_string( $db, $_POST['potencia'] );
    $velocidad = mysqli_real_escape_string( $db, $_POST['velocidad'] );
    $fecha = date('Y/m/d');
    ...
    if(!$modelo) {
        $validacion[] = "El modelo es obligatorio!!";   
    }
    if(!$caracteristicas) {
    ...
    if(!$potencia) {
        $validacion[] = "La potencia es obligatoria!!";    
    }
    if(!$velocidad) {
        $validacion[] = "La velocidad es obligatoria!!";    
    }
    ...
    if(empty($validacion)) {
        ...
        $query = "UPDATE patinetes 
        SET marca = '${marca}', modelo = '${modelo}', caracteristicas = '${caracteristicas}',
        precio = ${precio}, imagen = '${nombre_unico}', potencia = ${potencia}, velocidad = ${velocidad} WHERE id = ${id} ";
        ...
        if($consulta) {
           header('Location:../patinetes.php?consulta=2');
        }
    }  
            ...
            <form class="formulario" method="POST"  enctype="multipart/form-data">
                <fieldset>
                    <legend>Marca y modelo</legend>
                        ...
                        <input type="number" id="velocidad" name="velocidad" placeholder="Velocidad" min="10" max="100" value="<?php echo $velocidad; ?>">                    
