<?php 
    ...
    if($_SERVER ['REQUEST_METHOD'] ==='POST') {
        ...
        if($id) {
            ...
            $query = "SELECT imagen FROM repuestos WHERE id = ${id}";
           ...
           unlink('../imagenes/' . $img_delete['imagen']);
            ...
            if($consulta) {
                header('Location: repuestos.php?consulta=3');
            }            
        }
    }
...
<main class="contenedor-panel">
    ...
    <h1>Todos los repuestos y recambios</h1>
  ...
  <path d="M7 10h3v-3l-3.5 -3.5a6 6 0 0 1 8 8l6 6a2 2 0 0 1 -3 3l-6 -6a6 6 0 0 1 -8 -8l3.5 3.5" />
    ...
    <table class="tabla-productos">
        ...
        <tbody> <!--Listar los productos-->
            <?php while( $repuestos = mysqli_fetch_assoc($consultaDb)): ?>
            <tr>
                <td><?php echo $repuestos['id']; ?></td>
                <td><?php echo $repuestos['marca']; ?></td>
                <td><?php echo $repuestos['caracteristicas']; ?></td>
                <td><?php echo $repuestos['precio']; ?>€</td>
                <td><img src="../imagenes/<?php echo $repuestos['imagen']; ?>" class="img-tabla"> </td>
                <td><?php echo $repuestos['nombre']; ?>W</td>
                <td><?php echo $repuestos['fecha']; ?></td>

                <td>
                    <form method="POST" class="eliminar-form">
                        <input type="hidden" name="id" value="<?php echo $repuestos['id'] ?>">
                        <input type="submit" class="btn-eliminar" value="Eliminar">
                    </form>             
                    <a href="patinetes/actualizar-repuesto.php?id=<?php echo $repuestos['id']; ?>" class="btn-act">Editar</a>
                </td>
            </tr>
            <?php endwhile; ?>
        </tbody>
